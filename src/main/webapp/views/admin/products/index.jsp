		...
		<article class="row col-11">
			<div class="table-responsive col-10 offset-1">
				<a class="btn btn-success"
					href="/SOF3021_Assignment_Canhnhph17851/admin/products/create">
					Create </a>
				<table class="table">
					<thead>
						<tr>
					...
					<tbody>
						<c:forEach items="${ data.content }" var="pro">
							<tr>
								<td>${ pro.id }</td>
								<td>${ pro.name }</td>
								<td><img src="${ pro.image }" height="2cm" width="3cm" /></td>
								<td>${ pro.price }</td>
								<td>${ pro.createdDate }</td>

								<td>${ pro.available == 0 ? "In-Available" : "Available" }</td>
								<td>${ pro.category.name}</td>
								<td><a class="btn btn-primary"
									href="/SOF3021_Assignment_Canhnhph17851/admin/products/edit/${ pro.id }">
										Update </a></td>
									...
									<div class="modal-content">
										...
										<div class="modal-footer">
											...
											<a type="button" class="btn btn-primary"
												href="/SOF3021_Assignment_Canhnhph17851/admin/products/delete/${ pro.id }">Delete</a>
										</div>
									</div>
								</div>
							</div>
