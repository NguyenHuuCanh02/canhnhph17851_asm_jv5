		...
		<article class="row col-9">
			<div class="table-responsive col-10 offset-1">
				<a class="btn btn-success"
					href="/SOF3021_Assignment_Canhnhph17851/admin/orderDetails/createe">
					Create </a>
				<table class="table">
					<thead>
						<tr>
					...
					<tbody>
						<c:forEach items="${ tai.content }" var="osder">
							<tr>

								<td>${ osder.id }</td>
								<td>${ osder.order.user.fullname}</td>
								<td>${ osder.product.name }</td>
								<td>${ osder.price}</td>
								<td>${ osder.quantity}</td>
								<td><a class="btn btn-primary"
									href="/SOF3021_Assignment_Canhnhph17851/admin/orderDetails/edit/${osder.id}">
										Update </a></td>
								<td>><a class="btn btn-danger" data-bs-toggle="modal"
									data-bs-target="#exampleModal${ osder.id }"
									class="btn btn-primary">Detete</a>
								</td>
							</tr>
							<div class="modal fade" id="exampleModal${ osder.id }"
									...
									<div class="modal-content">
										...
										<div class="modal-footer">
											...
											<a type="button" class="btn btn-primary"
												href="/SOF3021_Assignment_Canhnhph17851/admin/orderDetails/delete/${osder.id}">Delete</a>
										</div>
									</div>
								</div>
							</div>
				...
				<div class="row m-0">
					<ul class="pagination">
						<c:forEach var="index" begin="0" end="${ tai.totalPages - 1 }">
							<li class="page-item"><a class="page-link"
								href="/SOF3021_Assignment_Canhnhph17851/admin/products/index?page=${ index }">${ index + 1 }</a>
							</li>
						</c:forEach>
					</ul>
				</div>
