	...
	<div class="row">
		...
		<article class="row col-9">
			<form:form method="POST" class="form-control"
				action="/SOF3021_Assignment_Canhnhph17851/admin/orderDetails/update/${orderDetail.id}"
				modelAttribute="orderDetail">
				<div>
					<label class="col-1">Order</label>
					<form:select path="order" name="order">
						<form:option value="${ orderDetail.order.id }">${ orderDetail.order.user.fullname}</form:option>
					</form:select>
				</div>
				<div>
					<label class="col-1">Product</label>
					<form:select path="product" name="product">
						<form:option value="${ orderDetail.product.id }">${ orderDetail.product.name }</form:option>
					</form:select>
				</div>
				<div>
					<label class="col-1">Product</label>
					<form:select path="price" name="price">
						<form:option value="${ orderDetail.product.price }">${ orderDetail.product.price }</form:option>
					</form:select>
				</div>
				<div>
					<label class="col-1">Quantity</label>
					<form:input path="quantity" value="${ orderDetail.quantity}" />
				</div>
				<div>
					<label class="col-1"> </label>
					<button class="pb-1 row btn btn-primary">Cập Nhật</button>
				</div>
			</form:form>
